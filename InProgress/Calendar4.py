...
i561 = Namespace("http://courseweb.ischool.illinois.edu/lis/2017fa/is561/")
...
ulist = []  # List of units
for s in mygraph.subjects(RDF.type, onto.Unit):
        myUnit = course.Unit(s)
        ulist.append(myUnit)
        for p,o in mygraph.predicate_objects(s):
                myUnit.setAttrib(p,o)

mlist = []  # List of meetings and holidays
for s in mygraph.subjects(RDF.type, onto.Meeting):
        myMeeting = course.Meeting(s)
        mlist.append(myMeeting)
        for p,o in mygraph.predicate_objects(s):
                myMeeting.setAttrib(p,o)

for s in mygraph.subjects(RDF.type, onto.Holiday):
        myHoliday = course.Meeting(s)              # Use Meeting class for holiday objects
        mlist.append(myHoliday)
        for p,o in mygraph.predicate_objects(s):
                myHoliday.setAttrib(p,o)
